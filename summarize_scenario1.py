...
from heft.heft.heft import (
    ...
    _compute_load_balance as common_load_balance,
    _compute_communication_cost as heft_comm_cost,
    _compute_waiting_time as common_wait_time,
)
...
for prefix, rc in PREFIXES:
    ...
    # Startup row check (square => zeros startup)
    ...
    per_proc_busy_heft, _, _, _ = common_load_balance(proc_sched_heft)
    ...
    wait_heft = common_wait_time(proc_sched_heft)
    # Energy
    energy_heft = 0.0
...
out_csv = os.path.join(BASE, 'scenario1_summary.csv')
