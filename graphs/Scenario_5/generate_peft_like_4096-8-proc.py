...
def main():
    ...
    ap.add_argument('--inherit-prefix')  # for uniform interface (not expected usually for base 8-proc)
    ...
    # Usually we generate fresh for base scenario
    if inherit_edges is not None:
        edges=inherit_edges
        print('Reutilizando conectividade herdada (base 8-proc).')
    else:
    ...
    if inherit_exec is not None:
        ...
        if base_cols>args.procs:
            print('[erro] base possui mais colunas que alvo', file=sys.stderr); sys.exit(1)
        diff=abs(exec_mat[:,:base_cols]-inherit_exec).max() if base_cols>0 else 0
