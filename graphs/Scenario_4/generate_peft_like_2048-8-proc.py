"""Generate a PEFT-like heterogeneous DAG with 2048 tasks and 8 processors.

Pattern and design philosophy replicate earlier scenarios (256, 512, 1024) scaled up:
- 7 layers (slightly more to keep per-layer size manageable) with forward-only edges.
- Each task chooses 1-3 outgoing edges into next 1-2 future layers (like earlier scripts allowing limited skips) to keep avg out-degree ~1.5.
- Communication weights: 0 (15% chance) else 5..60 then scaled modestly by layer depth.
- Execution: base uniform 8..40, per-processor perf multiplier 0.75..1.25, plus sparse task/processor affinity (mod 17 rule) lowering time.
- Power: correlated with inverse perf (speed) plus mild noise and slight bump for affinity speed-ups.
...
Usage example:
  python generate_peft_like_2048-8-proc.py --prefix graphs/Scenario_4/peft2048_8proc
"""
...
def write_connectivity(prefix:str, n_tasks:int, edges:list[tuple[int,int,int]]):
