"""Generate PEFT-like heterogeneous DAG (512 tasks, 8 processors) com opção de interface
uniforme de herança (para consistência com expansões >8). Para 8 procs normalmente
não há herança, mas aceitamos --inherit-prefix para coerência (ignora colunas maiores).
"""
...
def main():
    ...
    # Sobrescreve banda herdada no arquivo (mantendo resto) para consistência se houve herança parcial
    if inherited_cols>0 and inherit_bw is not None:
        ...
        header=rows[0]
        body=[[c for c in row] for row in rows[1:]]
        for i in range(inherited_cols):
